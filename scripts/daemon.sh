...
# =============================================================================
# wechat-opencode cross-platform daemon manager
# Supports: macOS (launchd) / Linux (systemd + nohup fallback)
# =============================================================================

DATA_DIR="${HOME}/.wechat-opencode"
...
macos_start() {
  ...
  # Collect provider env vars for plist
  local plist_extra_env=""
  for var in ANTHROPIC_AUTH_TOKEN ANTHROPIC_API_KEY ANTHROPIC_BASE_URL CLAUDE_API_KEY OPENAI_API_KEY OPENAI_BASE_URL GEMINI_API_KEY GOOGLE_GENERATIVE_AI_API_KEY; do
    if [ -n "${!var:-}" ]; then
...
linux_create_service_file() {
  ...
  # Collect provider env vars to pass through to the service
  local extra_env=""
  for var in ANTHROPIC_AUTH_TOKEN ANTHROPIC_API_KEY ANTHROPIC_BASE_URL CLAUDE_API_KEY OPENAI_API_KEY OPENAI_BASE_URL GEMINI_API_KEY GOOGLE_GENERATIVE_AI_API_KEY; do
    if [ -n "${!var:-}" ]; then
...
Documentation=https://github.com/Wechat-ggGitHub/wechat-opencode
