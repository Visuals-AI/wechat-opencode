...
scriptPath = fso.BuildPath(projectDir, "reset-windows.cmd")
logDir = shell.ExpandEnvironmentStrings("%USERPROFILE%") & "\.wechat-opencode\logs"
logPath = fso.BuildPath(logDir, "windows-reset.log")

EnsureFolder logDir

cmd = "cmd.exe /c " & Quote(Quote(scriptPath) & " --yes > " & Quote(logPath) & " 2>&1")
